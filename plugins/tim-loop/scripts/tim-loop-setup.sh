...
build_prompt() {
    ...
    if [[ -n "$REVIEW_FILE" && "$REVIEW_MODE" == "tech-review" ]]; then
        ...
        prompt+="Output <promise>$COMPLETION_PROMISE</promise> only when:\n"
        ...
        prompt+="3. You have no outstanding concerns\n\n"
        prompt+="If the plan has drifted from the original goal, do NOT output the completion promise. Instead, document the drift and recommend the plan be sent back to drafts.\n"
        ...
        prompt+="Iterate through all three phases until the plan is ready. If you have concerns about technical accuracy, AI-readiness, OR goal alignment, address them before completing.\n\n"
        prompt+="Output <promise>$COMPLETION_PROMISE</promise> only when:\n"
