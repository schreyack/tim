...
build_prompt() {
    ...
    if [[ -n "$REVIEW_FILE" && "$REVIEW_MODE" == "tech-review" ]]; then
        ...
        prompt+="### Items Marked as 'Already Done'\n"
        ...
        prompt+="**This is NOT a quick high-level review.** Read the actual code. Trace the actual logic. Run the actual tests. The human marked it 'done' based on their understanding at the time — your job is to verify that understanding was correct.\n\n"
        ...
        prompt+="- Code written but not integrated/imported\n"
        ...
        prompt+="If verification reveals gaps, **fix them**. The item isn't done until it's actually done.\n\n"
