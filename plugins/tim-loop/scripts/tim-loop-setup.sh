...
build_prompt() {
    ...
    if [[ -n "$REVIEW_FILE" ]]; then
        prompt+="## Mode: Review\n\n"
        prompt+="You are reviewing $REVIEW_FILE to validate it is complete, clear, and implementable.\n\n"
        prompt+="### Critical Rules\n"
        prompt+="1. **DO NOT change, reduce, or significantly shift the plan's scope** - Your job is to improve clarity and completeness, not to redesign the plan\n"
        prompt+="2. **If you want to suggest scope changes, STOP and ask the human** - Changes to goals, major implementation approach, or removing items require human approval\n"
        prompt+="3. **Your objective is to make this plan implementable** - Not to find reasons why it can't be done or shouldn't be done\n"
        prompt+="4. **If you are less than 98% confident on any decision, ask the human**\n\n"
        prompt+="### Review Focus\n"
        prompt+="- Clarity: Are instructions unambiguous?\n"
        prompt+="- Completeness: Are all steps specified?\n"
        prompt+="- Testability: Can success be verified?\n"
        prompt+="- Feasibility: Are there blocking issues? (If so, ASK - don't remove items)\n\n"
        prompt+="Iterate until the plan is ready for implementation. Do not declare completion if you have unresolved concerns - raise them instead.\n\n"
        ...
        prompt+="4. **Your objective is to accomplish the goal** - Not to find reasons why items can't be done, shouldn't be done, or are \"out of scope\"\n"
        ...
        prompt+="### Plan Quality\n"
        prompt+="- Steps should be specific enough that there's only one way to interpret them\n"
        prompt+="- Avoid weasel words like \"consider\", \"optionally\", \"if needed\" - be definitive\n"
        prompt+="- If you're unsure whether something should be included, ask - don't omit it\n\n"
        ...
        prompt+="Review your plan for clarity, completeness, and implementability.\n\n"
        prompt+="**Validation rules:**\n"
        prompt+="- DO NOT remove items to make the plan \"simpler\"\n"
        prompt+="- DO NOT weaken criteria to make them easier to pass\n"
        prompt+="- If you find issues, fix them or ask the human - don't delete the problematic item\n"
        prompt+="- Improve clarity without reducing scope\n\n"
        prompt+="Change to \`<!-- REVIEWED: YES -->\` when the plan is ready for implementation.\n\n"
        prompt+="---\n\n"
        prompt+="### Phase 3: Implement\n"
        prompt+="Execute 100% of the plan exactly as written.\n\n"
        ...
        prompt+="Verify that 100% of objectives are met.\n\n"
        prompt+="**Verification rules:**\n"
        prompt+="- Check each Goal - fully achieved, not \"mostly\" or \"partially\"\n"
        prompt+="- Check each Implementation Step - executed completely\n"
        prompt+="- Check each Test - exists and passes\n"
        prompt+="- Check each Completion Criterion - unambiguously true\n\n"
        prompt+="If ANY item is incomplete: fix it, don't rationalize why it's \"good enough\".\n\n"
        prompt+="Change to \`<!-- VERIFIED: YES -->\` only when everything is genuinely complete.\n\n"
        prompt+="---\n\n"
