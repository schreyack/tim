...
build_prompt() {
    ...
    if [[ -n "$REVIEW_FILE" ]]; then
        ...
        prompt+="- **Maintain complexity** - complex tasks need complex plans\n"
        prompt+="- **Clarify, don't redesign** - improve clarity while keeping direction\n"
        prompt+="- **Stay within scope** - no additions, no removals\n\n"
        ...
        prompt+="2. **Ask before changing scope** - Any scope change (additions, removals, approach changes) needs explicit approval\n"
        prompt+="3. **Ask when uncertain** - Multiple valid approaches? Unclear requirements? Ask.\n"
        ...
        prompt+="5. **Ask when uncertain** - Multiple valid approaches? Unclear requirements? Ask.\n\n"
        prompt+="### Common Shortcuts to Avoid\n"
        prompt+="These patterns often appear when work feels redundant or difficult:\n\n"
        prompt+="- **'Effectively done'** → Do it explicitly; implicit completion isn't verifiable\n"
        prompt+="- **'Already working'** → Verify against the plan's specific criteria\n"
        prompt+="- **'Out of scope'** → The plan defines scope; ask if you think something should change\n"
        prompt+="- **'I interpret this as...'** → When in doubt, ask rather than assume less work\n"
        prompt+="- **'Started'/'attempted'** → Completion means finished, not begun\n"
        prompt+="- **'Not needed because...'** → Each step exists for a reason; do it or ask why it's there\n"
        prompt+="- **'Previous step covers this'** → Steps are explicit; each one matters\n\n"
        prompt+="WHY: Plans are designed by humans who thought carefully about what they want.\n"
        prompt+="Shortcuts that seem clever often miss the human's actual intent.\n\n"
        ...
        prompt+="### Guiding Principles\n"
        prompt+="1. **Preserve the scope exactly** - The task defines the work you'll complete\n"
        prompt+="2. **Ask before changing scope** - Any scope change needs explicit approval\n"
