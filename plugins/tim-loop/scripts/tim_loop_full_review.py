...
PHASE_SIGNALS = {
    1: "PHASE-1-TECH-DONE",
    2: "PHASE-2-AI-READY-DONE",
    3: "PHASE-3-GOAL-ALIGN-DONE",
}
...
def get_phase_2_prompt(review_file: str) -> str:
    """Generate the Phase 2 (AI-Ready Review) prompt."""
    return f"""## Phase 2: AI-Ready Review
...
**To signal Phase 2 completion:** Output `<promise>PHASE-2-AI-READY-DONE</promise>`
"""


def get_phase_3_prompt() -> str:
    """Generate the Phase 3 (Goal Alignment) prompt."""
    return """## Phase 3: Goal Alignment Check (The Spirit Test)
...
**If the answer is YES** - the plan is ready for implementation.

**To signal Phase 3 completion:** Output `<promise>PHASE-3-GOAL-ALIGN-DONE</promise>`

After Phase 3, output `<promise>FULL-REVIEW-DONE</promise>` to complete the full review.
"""
...
def get_phase_prompt(phase: int, state: dict) -> str:
    ...
    if phase == 2:
        return get_phase_2_prompt(review_file)
    elif phase == 3:
        return get_phase_3_prompt()
    return ""
...
def handle_full_review_phase(
    ...
    if f"<promise>{final_promise}</promise>" in assistant_text:
        if current_phase < 3 or state.get("PHASE_3_COMPLETE") != "true":
    ...
    # Phase 3 complete - instruct to output final signal
    save_state(state)
