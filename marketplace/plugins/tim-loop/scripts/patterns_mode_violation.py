...
def build_mode_violation_response(violations: list[ModeViolation]) -> dict:
    """Build the block response for mode violations."""
    if not violations:
        return {}

    v = violations[0]  # Use first violation for the message
    mode_descriptions = {
        "tech-review": "technical review (checking accuracy, edge cases, testability)",
        "ai-ready": "AI-readiness review (ensuring unambiguous instructions)",
        "full-review": "comprehensive review (tech + AI-ready + goal alignment)",
        "pm-review": "PM review (organizing the plan, NOT changing scope)",
        "verify": "verification audit (checking if implementation is complete)",
    }

    mode_desc = mode_descriptions.get(v.current_mode, v.current_mode)

    message = f"""## Mode Violation Detected

**You are in `--{v.current_mode}` mode**, which is a REVIEW mode.

Your task is: {mode_desc}

**But you tried to:** `{v.matched_text}`

This is NOT an implementation task. You should be:
- Reading and analyzing the plan
- Checking for issues and improvements
- Making the plan clearer and more precise
- NOT writing code or implementing anything
...
**To continue:** Go back to the review task. Read the plan, identify issues, suggest improvements.

If you genuinely believe you've completed the review, output `<promise>{v.current_mode.upper().replace('-', '-')}-DONE</promise>` (for tech-review, ai-ready) or the appropriate completion tag.

Do NOT attempt to implement during review modes.
"""
