...
**Template:** Read the report template from `templates/PBT-REPORT.md` (relative to the tim-pbt plugin directory). Fill in the placeholder values with actual project data. The template includes the property type legend with codes (RT, ID, OI, etc.) — use these codes in the Coverage table.
...
After completing each module's property evaluation, update `bugs/.pbt-state.json`: add the property type code (e.g., `"RT"`, `"FV"`) to that module's `evaluated` array, increment `properties_tested`, and set `complete: true` when all 13 types have been evaluated for that module. Also update `bugs` array and `total_bugs_found` when filing bug reports.
...
```json
{
  ...
  "modules": {
    "core/models": {
      "files": 8,
      "evaluated": [],  // property type codes: RT,ID,OI,CA,CC,CF,FV,BO,TC,NP,SV,CO,SM
      "properties_tested": 0,
      "bugs_found": 0,
      "complete": false
    }
...
### Classic Property Types: check in this order

**RT — Round-trip / Inverse**
...
**CA — Commutativity / Associativity**
...
**SM — State Mutation Side Effects**
...
- **Classic properties (RT–CF):** only mine what code claims to have. Evidence: docstrings, type hints, function names, return type annotations, comments, or obvious semantics.
- **Bug-pattern properties (FV–SM):** actively scan code bodies for anti-patterns. These target what code *does wrong*, not what it *claims*.
...
- If a function has no testable properties, skip it. Do not force properties onto code that doesn't claim them (classic RT–CF) or that doesn't exhibit anti-patterns (bug-pattern FV–SM).
